...
export default function navbar({ lang }: { lang: string }) {
  ...
  const wordings: Record<string, string[]> = {
    en: ["Skip to content", "Who I Am", "What I Do", "Journal"],
    es: ["Saltar al contenido", "Quién Soy", "Qué Hago", "Diario"],
  };

  return html`
    <nav
      class="px-4 sm:pl-6 sm:pr-8 top-0 pt-2 md:pt-4 pb-2 mb-4 md:mb-0 flex flex-wrap mt-4 md:mt-12 justify-between w-full max-w-[60rem] m-auto bg-stone-100/70 backdrop-blur-sm dark:bg-stone-800/80 border-b border-stone-400 z-50 light-gradient-projection !sticky before:border-b-[2px] before:-bottom-[1px] before:border-green-300 dark:before:border-blue-700 before:!rotate-0 before:blur-0 [clip-path:polygon(0_0,100%_0,100%_100%,0_100%)]"
    >
      <a
